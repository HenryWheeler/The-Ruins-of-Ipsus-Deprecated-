...
namespace TheRuinsOfIpsus
{
    public static class ShadowcastFOV
    {
        ...
        static void Compute(uint octant, int oX, int oY, int rangeLimit, int x, Slope top, Slope bottom)
        {
            for (; (uint)x <= (uint)rangeLimit; x++) // rangeLimit < 0 || x <= rangeLimit
            {
                ...
                for (int y = topY; y >= bottomY; y--)
                {
                    ...
                    if (inRange) SetVisible(tx, ty, true);
        ...
        {
            foreach (Tile tile in visibleTiles) { Coordinate coordinate = tile.GetComponent<Coordinate>(); SetVisible(coordinate.x, coordinate.y, false); }
            visibleTiles.Clear();
        }
        public static void SetVisible(int x, int y, bool visible, bool all = false)
        {
            if (CMath.CheckBounds(x, y))
            {
                if (visible)
                {
                    ...
                    if (!all) { visibleTiles.Add(Map.map[x, y]); }
