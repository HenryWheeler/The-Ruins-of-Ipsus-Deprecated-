...
namespace TheRuinsOfIpsus
{
    ...
    public class InflictPoisonOnHit : OnHitProperty
    {
        ...
        {
            if (target.GetComponent<Poison>() == null) { target.AddComponent(new Poison(strength * 3, strength));
                Log.AddToStoredLog(attacker.GetComponent<Description>().name + " have inflicted " + target.GetComponent<Description>().name + " with a malignant Green*poison"); }
        }
